...
| Coach mobile | `apps/coach` (`@darkmat/coach`) depending on `@darkmat/shared` |
| Shared package | `@darkmat/shared` — types, auth/roles, repositories, theme tokens, **RN UI primitives**, RN Auth/Theme providers |
...
1. Importing `@darkmat/shared` barrel pulls RN UI into Next.js → bundle/SSR failure
...
4. Splitting `@darkmat/shared` into many packages in one pass risks Coach mobile breakage
...
| Typography without `react-native` `TextStyle` | `@darkmat/shared/theme` must be importable from web |
