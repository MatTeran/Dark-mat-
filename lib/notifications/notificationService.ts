...
const ANDROID_CHANNEL_ID = 'dark-mat-default';
  ...
  if (existing.status !== 'granted') {
    if (existing.canAskAgain === false) {
      return {
        status: 'blocked',
        message:
          'Notifications are turned off for Dark Mat. Enable them in device Settings.',
...
export async function scheduleClassReminder(
  ...
  const data: DarkMatNotificationData = {
...
export async function showStreakAtRiskNotification(input: {
  userId: string;
}): Promise<DarkMatNotification | null> {
  const prefs = await loadNotificationPreferences();
...
export function parseNotificationData(
  raw: unknown,
): DarkMatNotificationData | null {
  if (!raw || typeof raw !== 'object') {
    return null;
  }
  const data = raw as Partial<DarkMatNotificationData>;
