...
const PREFS_KEY = '@dark-mat/notification-preferences';
const HISTORY_KEY = '@dark-mat/notification-history';
const PROMPT_KEY = '@dark-mat/notification-permission-prompt';
const REMINDER_MAP_KEY = '@dark-mat/class-reminder-ids';
...
export async function loadNotificationHistory(): Promise<DarkMatNotification[]> {
  const raw = await AsyncStorage.getItem(HISTORY_KEY);
  const list = safeParse<DarkMatNotification[]>(raw, []);
  return Array.isArray(list) ? list : [];
}

export async function saveNotificationHistory(
  items: DarkMatNotification[],
): Promise<void> {
...
export async function prependNotificationHistory(
  item: DarkMatNotification,
): Promise<DarkMatNotification[]> {
