...
export interface ForegroundBannerState {
  ...
  title: string;
  body: string;
  category: DarkMatNotification['category'];
}

interface NotificationContextValue {
  ready: boolean;
  preferences: NotificationPreferences;
  history: DarkMatNotification[];
  unreadCount: number;
  ...
  addToHistory: (item: DarkMatNotification) => Promise<void>;
  handleNotificationNavigation: (data?: DarkMatNotificationData | null) => void;
...
export function NotificationProvider({ children }: PropsWithChildren) {
  ...
  const [history, setHistory] = useState<DarkMatNotification[]>([]);
  ...
  const addToHistory = useCallback(async (item: DarkMatNotification) => {
  ...
  const handleNotificationNavigation = useCallback(
    (data?: DarkMatNotificationData | null) => {
  ...
  const runDevTest = useCallback(
    async (kind: DevTestKind) => {
      const map: Record<
        DevTestKind,
        { title: string; body: string; data: DarkMatNotificationData }
      > = {
        class_reminder: {
