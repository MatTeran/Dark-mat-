...
import type { DarkMatNotification } from '../../lib/notifications';
...
function groupNotifications(items: DarkMatNotification[]): HistoryGroup[] {
  const now = new Date();
  const today = startOfDay(now).getTime();
  const yesterday = today - 24 * 60 * 60 * 1000;

  const buckets: Record<string, DarkMatNotification[]> = {
    today: [],
    yesterday: [],
    earlier: [],
  };
...
function actionLabelFor(item: DarkMatNotification): string | undefined {
